...
public class Classroom {
    ...
    public void enterClassroom ( String filename ) {
        ...
        for (int i = 0; i < count; i++){
            ...
            studentsInLine = new SNode(student, second);
        }}
    }
            ...
            for (int j = 0; j < studentsSitting[i].length; j++){
                if (studentsSitting[i][j] != null){ 
                    if (musicalChairs == null){
                        ...
                        first.setNext(musicalChairs);
                        ptr = first;
                        first.setStudent(studentsSitting[i][j]);
    ...
    public void moveStudentFromChairsToLine(int size) {
        ...
        if (ptr == musicalChairs){ // last position
            insertByHeight(ptr.getStudent());
            ptr2.setNext(musicalChairs.getNext());
            musicalChairs = ptr2;
            // musicalChairs = new SNode();
            // musicalChairs = ptr2;
            // musicalChairs.setNext(ptr.getNext());
        } else if (ptr == musicalChairs.getNext()) { //first position
            insertByHeight(ptr.getStudent());
            musicalChairs.setNext(ptr.getNext());
    ...
    public void insertByHeight(Student studentToInsert) {
        ...
        // WRITE YOUR CODE HERE
        SNode newStudent = new SNode(studentToInsert, null);
        SNode ptr = studentsInLine;
        SNode temp = new SNode();

        if (studentsInLine == null){ // when the line begins
            studentsInLine = newStudent;
        } 
        else if (studentToInsert.getHeight() < studentsInLine.getStudent().getHeight()){ // shortest, front of the line
            newStudent.setNext(studentsInLine);
            studentsInLine = newStudent;
        } else { 
            while (ptr.getNext() != null){
                if (ptr.getNext().getStudent().getHeight() > studentToInsert.getHeight()){
                    break;
                } else{
                 ptr = ptr.getNext();
            // traverses through the list until student is in the right spot
                } 
            }
            temp = ptr.getNext();
    ...
    public void seatMusicalChairsWinner () {
        ...
        // WRITE YOUR CODE HERE
        if (musicalChairs.getNext() == musicalChairs && musicalChairs != null){
            for (int i = 0; i < seatingLocation.length; i++){
                for (int j = 0; j < seatingLocation[i].length; j++){
